...
const $$PostCard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<article class="flex w-full flex-col gap-4 md:max-w-2xl md:flex-row md:items-center"> <div class="relative h-[240px] w-full overflow-hidden rounded-[31px] lg:max-w-[289px]"> ${renderComponent($$result, "Image", $$Image, { "src": data.hero_image, "alt": data.hero_alt, "format": "webp", "quality": "high", "loading": "eager", "class": "h-full w-full object-cover" })} </div> <div class="flex flex-col space-y-4 py-4 md:h-full"> <h3 class="text-xl font-semibold uppercase text-[#F5A898]">${data.title}</h3> <p class="flex-1 md:max-w-sm">${data.description}</p> ${renderComponent($$result, "Button", $$Button, { "href": `/blogs/${slug}`, "text": "Lire la suite", "className": "text-semibold w-fit border border-orange bg-[#f5a798a6] text-orange hover:bg-[#f5a7986e] dark:text-white" })} </div> </article>`;
...
const $$Index$1 = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { data } = posts[0];
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": "PossaCode - Blog" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="container py-40"> <h2 class="text-healding-2 font-semibold uppercase">Evenements</h2> <div class="w-full"> <dir class="relative mx-auto block w-full max-w-2xl overflow-hidden">${renderComponent($$result2, "Image", $$Image, { "src": data.hero_image, "alt": data.hero_alt, "loading": "eager", "format": "webp", "quality": "high", "class": "cover w-full rounded-xl" })}</dir> </div> <div class="mt-10"> ${posts && posts.slice(1).map((post) => renderTemplate`<a${addAttribute(`/blogs/${post.slug}`, "href")}> ${renderComponent($$result2, "PostCard", $$PostCard, { ...post })} </a>`)} </div> </div> ` })}`;
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Index;
  const LastPosts = (await getCollection("blog")).sort((a, b) => a.data.pubDate.valueOf() - b.data.pubDate.valueOf()).slice(0, 4);
...
« Le programme accroît la moyenne scolaire. Les élèves sont plus
            ...
            aspirations des élèves, à la fois en matière d’études et de
            profession. »*
</p> ${renderComponent($$result2, "Button", $$Button, { "href": "/", "text": "Lire la suite", "className": "max-w-[142px] border-[0.7px] border-white/50 bg-white/15 transition hover:bg-white/20", "data-astro-cid-j7pv25f6": true })} </div> </div> <div class="grid grid-cols-1 gap-10 md:grid-cols-2" data-astro-cid-j7pv25f6> ${LastPosts.map((post) => renderTemplate`${renderComponent($$result2, "PostCard", $$PostCard, { ...post, "data-astro-cid-j7pv25f6": true })}`)} </div> ${LastPosts.length >= 4 && renderTemplate`${renderComponent($$result2, "Button", $$Button, { "href": "/blogs", "text": "Toutes les actualis\xE9s", "className": "mx-auto block w-fit", "data-astro-cid-j7pv25f6": true })}`} </div> </section> <section id="sponsors" class="pb-10 md:pb-44" data-astro-cid-j7pv25f6> <div class="container space-y-14 md:space-y-20" data-astro-cid-j7pv25f6> <h2 class="text-healding-2 font-semibold uppercase" data-astro-cid-j7pv25f6>
